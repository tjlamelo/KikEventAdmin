...
type ToastType = 'success' | 'error' | 'warning' | 'danger' | 'info';
...
export class ToastService {
  private toastConfig: Record<ToastType, ToastConfig> = {
    success: {
    ...
    danger: {
      ...
      icon: `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm-1-11a1 1 0 112 0v4a1 1 0 11-2 0V7zm1 8a1.5 1.5 0 100-3 1.5 1.5 0 000 3z" clip-rule="evenodd"/></svg>`,
    },
  };

  private toasts: Toast[] = [];

  show(type: ToastType, message: string, timeout: number = 5000) {
    const toast: Toast = { type, message, timeout };
